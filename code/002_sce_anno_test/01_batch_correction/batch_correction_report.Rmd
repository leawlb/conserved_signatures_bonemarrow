...
if(batch_use == "Object_ID"){
  ...
  sce_mnn <- readRDS(file = snakemake@input[["sce_input_corrected"]][[1]])
  sce_ser <- readRDS(file = snakemake@input[["sce_input_corrected"]][[2]])
  sce_list <- list(list(sce_input_raw, " after merging"),
                   list(sce_mnn, " MNN batch correction"),
...
sce_input_raw <- cms(sce_input_raw, group = "Batch_exp_day", k = 30,
              assay_name = "logcounts", res_name = "Logcounts")
visHist(sce_input_raw, n_col = 3)
...
max_obj <- length(unique(sce_mnn$Object_ID))
max_bed <- length(unique(sce_mnn$Batch_exp_day))
max_bsq <- length(unique(sce_mnn$Batch_sequencing))
