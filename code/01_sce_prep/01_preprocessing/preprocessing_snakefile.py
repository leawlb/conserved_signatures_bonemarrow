...
OUTPUT_REP = "/reports/01_sce_prep/01_preprocessing/"
...
for s in species:
  for i in individuals:
    if s in i:
      targets = targets + [OUTPUT_BASE + OUTPUT_DAT + "01_drop/" + s + "/sce_" + i + "-01"]
      targets = targets + [OUTPUT_BASE + OUTPUT_DAT + "02_outl/" + s + "/sce_" + i + "-02"]
      targets = targets + [OUTPUT_BASE + OUTPUT_DAT + "03_norm/" + s + "/sce_" + i + "-03"]
      targets = targets + [OUTPUT_BASE + OUTPUT_DAT + "04_dimr/" + s + "/sce_" + i + "-04"]
      targets = targets + [OUTPUT_BASE + OUTPUT_REP + s + "/preprocessing_sample_report_" + i + ".html"]
      
if config["run_preprocessing_summary"]:
  targets = targets + [OUTPUT_BASE + OUTPUT_REP + "preprocessing_summary.html"]
...
rule remove_droplets:
    ...
    output:
        sce_01 = OUTPUT_BASE + OUTPUT_DAT + "01_drop/{species}/sce_{individual}-01"
    params:
...
rule remove_outliers:
    ...
    output:
        sce_02 = OUTPUT_BASE + OUTPUT_DAT + "02_outl/{species}/sce_{individual}-02"
    params:
...
rule normalize_expr:
    ...
    output:
        sce_03 = OUTPUT_BASE + OUTPUT_DAT + "03_norm/{species}/sce_{individual}-03"
    script:
...
rule reduce_dims:
    ...
    output:
        sce_04 = OUTPUT_BASE + OUTPUT_DAT + "04_dimr/{species}/sce_{individual}-04" 
    params:
...
rule make_sample_reports:
    ...
    output:
        OUTPUT_BASE + OUTPUT_REP + "{species}/preprocessing_sample_report_{individual}.html"
    params:
        cutoff_umis = VALUES["cutoff_umis"],
        cutoff_doublets = VALUES["cutoff_doublets"],
        cutoff_sum = VALUES["cutoff_sum"],
...
if config["run_preprocessing_summary"]:
  rule make_summary_report:
      input:
          sce_01_path = OUTPUT_BASE + OUTPUT_DAT + "01_drop/"
      output:
          OUTPUT_BASE + OUTPUT_REP + "preprocessing_summary.html"
      params:
          cutoff_sum = VALUES["cutoff_sum"],
          cutoff_detected = VALUES["cutoff_detected"],
          cutoff_mitos = VALUES["cutoff_mitos"],
