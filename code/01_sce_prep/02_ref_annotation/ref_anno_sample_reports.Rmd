...
col_cts_baccin_cell <- col_cts_ref[match(levels(sce$baccin_celltype_scmapcell), 
                                     names(col_cts_ref))]
col_cts_baccin_clst <- col_cts_ref[match(levels(sce$baccin_celltype_scmapclust),
                                     names(col_cts_ref))]
col_cts_dahlin_cell <- col_cts_ref[match(levels(sce$dahlin_celltype_scmapcell), 
                                     names(col_cts_ref))]
col_cts_dahlin_clst <- col_cts_ref[match(levels(sce$dahlin_celltype_scmapclust), 
                                     names(col_cts_ref))]
col_cts_dolgalev_cell <- col_cts_ref[match(levels(sce$dolgalev_celltype_scmapcell),
                                       names(col_cts_ref))]
col_cts_dolgalev_clst <- col_cts_ref[match(levels(sce$dolgalev_celltype_scmapclust),
                                       names(col_cts_ref))]
```
...
if(! 1 %in% dim(mat_comp)){
  ...
  pheatmap::pheatmap(mat_comp_temp1, cluster_cols = FALSE, cluster_rows = FALSE, 
           main = paste0(name_curr, " Baccin overlap"))

  pheatmap::pheatmap(mat_comp_temp2, cluster_cols = FALSE, cluster_rows = FALSE, 
           main = paste0(name_curr, " Baccin overlap"))
}else{
