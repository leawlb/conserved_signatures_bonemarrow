---
title: "integration_report_fractions"
author: "Lea Wölbert"
date: '2023-06-14'
...
make_plots <- function(sce, title){
  ...
  col_cts_baccin_clst <- col_cts_ref[match(unique(sce$baccin_celltype_scmapclust),
                                       names(col_cts_ref))]
  col_cts_dahlin_clst <- col_cts_ref[match(unique(sce$dahlin_celltype_scmapclust), 
                                       names(col_cts_ref))]
  col_cts_dolgalev_clst <- col_cts_ref[match(unique(sce$dolgalev_celltype_scmapclust),
                                         names(col_cts_ref))]
  
  if(title == "species"){
    title <- paste(title, sce$Species_ID[1])
  }
...
```{r}
plotlist_fractions <- make_plots(sce_input, title = "fraction")
plotlist_fractions
```
