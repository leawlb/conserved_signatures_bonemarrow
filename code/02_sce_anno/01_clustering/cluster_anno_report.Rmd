...
cluster <- readRDS(file = snakemake@input[["sep"]])
cluster_curr <- snakemake@wildcards[["cluster"]]
print(cluster == cluster_curr)
...
col_cts_baccin <- col_cts_ref[match(levels(sce$baccin_celltype_scmapclust), 
                                names(col_cts_ref))]
col_cts_dahlin <- col_cts_ref[match(levels(sce$dahlin_celltype_scmapclust), 
                                names(col_cts_ref))]
col_cts_dolgalev <- col_cts_ref[match(levels(sce$dolgalev_celltype_scmapclust),
                                  names(col_cts_ref))]
