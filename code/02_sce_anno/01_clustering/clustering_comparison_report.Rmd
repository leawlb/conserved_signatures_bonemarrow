...
if(length(base::unique(sce_l$cluster_louvain)) >= 
   length(base::unique(sce_ls$cluster_louvain))){
  col_clust_l <- col_num[names(col_num) %in% sce_l$cluster_louvain]
}else{
  col_clust_l <- col_num[names(col_num) %in% sce_ls$cluster_louvain]
}
```
...
umap3 <- umap_base(sce_gg_cl, color_by = "species_cluster_louvain")+
  ggplot2::scale_color_manual(
    values = col_clust_l[
      names(col_clust_l) %in% sce_gg_cl$species_cluster_louvain])+
  ggplot2::ggtitle(base::paste(individual_curr,
...
ggplot2::ggplot(data = ggtable_cl,
  ...
  scale_fill_manual(values = col_clust_l)
```

```{r heatmap_clust}
ggtable_cl_heatmap <- ggtable_cl
ggtable_cl_heatmap$Var1 <- unfactor(ggtable_cl_heatmap$Var1)
ggtable_cl_heatmap$Var2 <- unfactor(ggtable_cl_heatmap$Var2)

ggplot2::ggplot(ggtable_cl_heatmap, aes(x = reorder(Var1, -Freq), 
                                        y = reorder(Var2, -Freq), 
                                        fill = Freq))+
  ggplot2::geom_tile()+
  ...
  ggplot2::scale_fill_continuous("Frequency", 
                                 ...
                                 breaks=seq(0,base::max(ggtable_cl$Freq), 
                                            by = 1000),
                                 low = "white", high = "red3")+
  ggplot2::ggtitle(individual_curr)+
  ggplot2::ylab("species clusters")+
  ggplot2::xlab("original clusters")
```
...
intermed_col_ct <- c(col_num[1:length(base::unique(sce_ct$annotation_cluster))], "grey86")
...
umap1 <- umap_base(sce_ct, color_by = "annotation_cluster")+
  ...
  scale_color_manual(values = intermed_col_ct)
...
umap3 <- umap_base(sce_gg_ct, color_by = "species_cluster_louvain")+
  ggplot2::scale_color_manual(
    values = col_clust_l[
      names(col_clust_l) %in% sce_gg_ct$species_cluster_louvain])+
  ggplot2::ggtitle(base::paste(individual_curr,
                               "species-specific cells, species clusters"))
UMAP3 <- ggpubr::ggarrange(umap3, legend_species)
UMAP3
...
base::table(sce_ls$cluster_louvain[match(
  colnames(sce_gg_ct),
  colnames(sce_ls))])

base::table(sce_ls$annotation_orig[match(
  colnames(sce_gg_ct),
  colnames(sce_ls))])

sce_ls$annotation_orig[match(
  colnames(sce_gg_ct),
...
ggplot2::ggplot(data = ggtable_ct, aes(axis1 = Var2, axis2 = Var1, y = Freq))+
  ...
  ggplot2::theme(legend.position = "none")+
  ggplot2::ggtitle(base::paste("species-specific", individual_curr))+
  scale_fill_manual("annotated", values = intermed_col_ct)
```

```{r heatmap_ct}
ggplot2::ggplot(ggtable_ct, aes(x = reorder(Var1, -Freq), 
                                y = reorder(Var2, -Freq), 
                                fill = Freq))+
  ggplot2::geom_tile()+
  ggplot2::theme_classic()+
  ...
  ggplot2::ggtitle(base::paste("species-specific",individual_curr))+
  ggplot2::ylab("species clusters")+
  ggplot2::xlab("original annotated cell types")
```
