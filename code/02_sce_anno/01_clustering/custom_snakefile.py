...
if config["run_custom_reports"]: 
  for f in fractions:
    ...
    targets = targets + [OUTPUT_REP + "/02_clustering/comparison_custom/custom_" + f + "_clustering_report_unif.html"]
...
# because the reports will run without waiting due to the fact that the data
# output is not the same as the snakemake rules output
print(config["run_custom_reports"])
if config["run_custom_reports"]: 
...
# HSC reports
    ...
    rule custom_hsc_clustering_report_unif:
        output:
            OUTPUT_REP + "/02_clustering/comparison_custom/custom_hsc_clustering_report_unif.html"
        resources:
            mem_mb = 70000,
            queue = "medium-debian"
        conda:
    ...
    rule custom_str_clustering_report_unif:
        output:
            OUTPUT_REP + "/02_clustering/comparison_custom/custom_str_clustering_report_unif.html"
        resources:
            mem_mb = 70000,
            queue = "medium-debian"
        conda:
