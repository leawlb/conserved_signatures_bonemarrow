...
hsc_clusters <- c(1:length(unique(sce_hsc$cluster_louvain)))
str_clusters <- c(1:length(unique(sce_str$cluster_louvain)))
...
for(i in hsc_clusters){
  ...
  print(output_path)
  saveRDS(hsc_clusters_list[[i]], file = output_path)
}
...
for(i in str_clusters){
  ...
  print(output_path)
  saveRDS(str_clusters_list[[i]], file = output_path)
}
