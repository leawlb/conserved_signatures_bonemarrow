...
targets = []
for f in fractions:
  targets = targets + [OUTPUT_DAT + "03_clst/hrchl_clust/sce_" + f + "-03"]
  targets = targets + [OUTPUT_DAT + "03_clst/seurt_clust/sce_" + f + "-03"]
  targets = targets + [OUTPUT_DAT + "03_clst/louvn_clust/sce_" + f + "-03"]
...
rule hierarchical_clustering:
    input: 
        sce_input = OUTPUT_DAT + "02_mnnc/sce_{fraction}_Object_ID-02"
    output:
        sce_03 = OUTPUT_DAT + "03_clst/hrchl_clust/sce_{fraction}-03"
    params:
        number_k = VALUES["number_k"],
        nr_hvgs = config["values"]["preprocessing"]["nr_hvgs"],
        size_subs_hscs = VALUES["size_subs_hscs"]
    script:
        "scripts/03_hierarchical_clustering.R"
        
rule seurat_clustering:
    input: 
        sce_input = OUTPUT_DAT + "02_mnnc/sce_{fraction}_Object_ID-02"
    output:
        sce_03 = OUTPUT_DAT + "03_clst/seurt_clust/sce_{fraction}-03"
    params:
        resolution = VALUES["resolution"]
    script:
        "scripts/03_seurat_clustering.R"
        
rule louvain_clustering:
    input: 
        sce_input = OUTPUT_DAT + "02_mnnc/sce_{fraction}_Object_ID-02"
    output:
        sce_03 = OUTPUT_DAT + "03_clst/louvn_clust/sce_{fraction}-03"
    params:
        k_louvain = VALUES["k_louvain"]
    script:
        "scripts/03_louvain_clustering.R"
