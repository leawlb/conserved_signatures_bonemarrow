...
col_cts_baccin_cell <- col_cts[names(col_cts) %in% sce$baccin_celltype_scmapcell]
col_cts_baccin_clst <- col_cts[names(col_cts) %in% sce$baccin_celltype_scmapclust]
col_cts_dahlin_cell <- col_cts[names(col_cts) %in% sce$dahlin_celltype_scmapcell]
col_cts_dahlin_clst <- col_cts[names(col_cts) %in% sce$dahlin_celltype_scmapclust]
col_cts_dolgalev_cell <- col_cts[names(col_cts) %in% sce$dolgalev_celltype_scmapcell]
col_cts_dolgalev_clst <- col_cts[names(col_cts) %in% sce$dolgalev_celltype_scmapclust]
...
plot_1 <- umap_base(sce, color_by = "dahlin_celltype_scmapcell")+ # own function
  ggtitle(name_curr)+
...
plot_1 <- umap_base(sce, color_by = "dolgalev_celltype_scmapcell")+ # own function
  ggtitle(name_curr)+
  scale_color_manual("dolgalev_celltype_scmapcell", values = col_cts_dolgalev_cell)
...
plot_2 <- umap_base(sce, color_by = "dolgalev_celltype_scmapclust")+ 
  ggtitle(name_curr)+
  scale_color_manual("dolgalev_celltype_scmapclust", values = col_cts_dolgalev_clst)
...
sce$Identity_ref_all <- sce$baccin_celltype_scmapclust
...
mat_comp <- as.matrix(table(sce$baccin_celltype_scmapcell,
                            sce$baccin_celltype_scmapclust))
...
mat_comp <- as.matrix(table(sce$dahlin_celltype_scmapcell,
                            sce$dahlin_celltype_scmapclust))
if(! 1 %in% dim(mat_comp)){
  mat_comp <- mat_comp[which(rowMeans(mat_comp) > 0), 
                       which(colMeans(mat_comp) > 0)]
...
mat_comp <- as.matrix(table(sce$dolgalev_celltype_scmapcell,
                            sce$dolgalev_celltype_scmapclust))
if(! 1 %in% dim(mat_comp)){
  mat_comp <- mat_comp[which(rowMeans(mat_comp) > 0), 
                       which(colMeans(mat_comp) > 0)]
