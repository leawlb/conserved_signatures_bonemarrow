...
k_louvain <- snakemake@params[["k_louvain"]] # original clustering parameters
fraction_curr <- snakemake@wildcards[["fraction"]]

#-------------------------------------------------------------------------------
# unite the required gene sets from all cell types
...
# use logcounts assay, which contains log-transformed, and normalized counts +
# normalized across samples using MultiBatchNorm

sce_cc <- runPCA(sce_cc, ncomponents=25, exprs_values = "logcounts") 
sce_nd <- runPCA(sce_nd, ncomponents=25, exprs_values = "logcounts") 
sce_cm <- runPCA(sce_cm, ncomponents=25, exprs_values = "logcounts") 

#-------------------------------------------------------------------------------
# cluster and add clusters to SCE object
