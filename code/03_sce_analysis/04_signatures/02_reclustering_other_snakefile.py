...
RUN_MARK_RAND_OTHER_PERM = config["run_mark_rand_other_permutation"] 
...
# run pval correction AT THE END, including all tests except zebrafish
...
datasets_other_hsc_nozeb = ["ts_hscs_progenitors", "ts_bone_marrow", "mus_weinreb_hspc", "mus_tm_bonemarrow"]
datasets_other_hsc_withzeb = ["ts_hscs_progenitors", "ts_bone_marrow", "mus_weinreb_hspc", "mus_tm_bonemarrow", "zeb_all_hspc"]
datasets_other_str = ["ts_all_stromal", "li_all_stromal", "mus_tik_stromal", "mus_bar_stromal"]

datasets_other_nozeb = datasets_other_hsc_nozeb + datasets_other_str
datasets_other_withzeb = datasets_other_hsc_withzeb + datasets_other_str
print(datasets_other_withzeb)

#-------------------------------------------------------------------------------

targets = []
     
for d in datasets_other_withzeb:
  targets = targets + [OUTPUT_DAT_OTHER + "/03_recl/reclustered_" + d + "_list"]
  targets = targets + [OUTPUT_DAT_OTHER + "/04_rcls/score_df_" + d + "_list"]
  targets = targets + [OUTPUT_REP + "/overview/reclustering_other_report_" + d + ".html"]
  ...
  if RUN_MMMS_RAND_OTHER_PERM:
    ...
    targets = targets + [OUTPUT_REP + "/mmusall_markers/perm_mmms_rand_" + d + ".html"]
...
# Using our own datasets + from other species (human, zebrafish)
# with:
#
# - signatures
...
#
# Standard Seurat approach with standard options starting from raw counts if
# possible but with aforementioned gene sets instead of HVGs.
#
# Ensembl DF lists still contain sub-clustering genes
"""
...
rule reclustering_other_report:
    ...
    output:
        OUTPUT_REP + "/overview/reclustering_other_report_{dataset}.html"
    resources:
          mem_mb=50000,
          queue="medium-debian"
    threads: 4
...
if RUN_MMMS_RAND_OTHER_PERM:
  ...
  # visualise reclustering permutation
  rule permutation_report_mmms_rand:
      input:
          seu_list = rules.reclustering_other.output,
          score_df_list = rules.reclustering_other_scores.output,
          perm_score_df = rules.perm_mark_rand_other.output
      ...
      resources:
          ...
          queue = "medium-debian"
      output:
          OUTPUT_REP + "/mmusall_markers/perm_mmms_rand_{dataset}.html"
      script: 
          "02_permutation_other_rand_report.Rmd"
...
# exluding zeb because it is not part of the main analysis or extended data
...
if RUN_PVAL_CORRECTION:
  ...
  rule pval_correction:
      input:
          ...
          other_sign_rand = expand(OUTPUT_DAT_OTHER + "/08_expp/sign-vs-rand_{dataset}", dataset = datasets_other_nozeb),
          other_mark_rand = expand(OUTPUT_DAT_OTHER + "/08_expp/mark-vs-rand_{dataset}", dataset = datasets_other_nozeb),
          other_mmms_rand = expand(OUTPUT_DAT_OTHER + "/08_expp/mmms-vs-rand_{dataset}", dataset = datasets_other_nozeb),
          other_mark_signrand = expand(OUTPUT_DAT_OTHER + "/08_expp/mark-vs-signrand_{dataset}", dataset = datasets_other_nozeb),
          other_mmms_signrand = expand(OUTPUT_DAT_OTHER + "/08_expp/mmms-vs-signrand_{dataset}", dataset = datasets_other_nozeb)
      output:
