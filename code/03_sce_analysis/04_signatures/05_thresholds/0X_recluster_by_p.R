...
signature_genes_by_p <- readRDS(base::paste0(
  threshold_path,
  "/signature_genes_by_p.rds"))

# lists of unique HSPC signature genes when sliding p value threshold 
# on nDEG analysis
names(signature_genes_by_p)
lapply(signature_genes_by_p, function(vec){print(length(vec))})
...
signature_genes_by_p_ensids <- lapply(signature_genes_by_p, function(vec){
...
subset_seu_list_by_p <- lapply(
  signature_genes_by_p_ensids, 
  function(ensids, seu){
    ...
    seu_sign@misc$used_genes <- "conserved_signature_treshold_testing_by_p"
