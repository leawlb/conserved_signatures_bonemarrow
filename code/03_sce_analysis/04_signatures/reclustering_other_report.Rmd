...
library(mclust, quietly = TRUE) # for adjusted rand index function
```
...
add_res_info <- function(seu_list){
  ...
  for(res in names(seu_list)){
    seu_list[[res]]$resolution <- base::rep(res, ncol(seu_list[[res]]))
  }
  return(seu_list)
}
```
...
vis_perc_wide_gg <- function(seu, correction_vec = NULL, title_add= NULL){
  ...
  mat1 <- mat/BiocGenerics::rowSums(mat)*100
  ...
  plot_wide <- ggplot2::ggplot(df_vis, aes(y = Var1, x = Var2, fill= Freq))+ 
    ...
    ggplot2::ggtitle(base::paste("resolution", seu$resolution[1], title_add))
  
   plot_wide_ordered <- ggplot2::ggplot(df_vis, 
                                        aes(y = reorder(Var1, -Freq),
                                            x = reorder(Var2, Freq),
                                            fill= Freq))+ 
    ggplot2::geom_tile()+
    ggplot2::scale_fill_continuous("% cells/cell type", 
  ...
  return(list(plot_wide, plot_wide_ordered))
}
```
...
vis_perc_long_gg <- function(seu, correction_vec = NULL, title_add= NULL){
  ...
  mat2 <- mat/BiocGenerics::rowSums(mat)*100
...
vis_clustering_values <- function(results_list, title){
  ...
  vis_df_1 <- tidyr::pivot_longer(vis_df_1, 
                                  cols = c("prop_zeros", "score", "sum"), 
                                  names_to = "type",
                                  values_to = "value")
...
```{r sign2, fig.width = 9, fig.height=3}
...
```{r markers2, fig.width = 9, fig.height=3}
...
```{r bl6markers2, fig.width = 9, fig.height=3}
