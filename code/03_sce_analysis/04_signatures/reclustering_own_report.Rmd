...
ggplot2::ggplot(
  res_df_all[!res_df_all$type %in% c("variation_information", 
                                     "nr_celltypes",
                                     "nr_clusters"),], 
...
ggplot2::ggplot(res_df_all[res_df_all$type == "variation_information",], 
                aes(y = value, x = type, color = conservation_level))+
  ggplot2::geom_point(alpha = 0.7)+
  ggplot2::theme_classic()+
  ggplot2::theme(axis.text.x = element_text(angle = 90))+
  ggplot2::scale_color_manual("conservation level", values = c(
    ...
    "mmusall_markers" = "blue2",
    "ndges" = "orangered1"))+
  ggplot2::ylim(c(0, max(res_df_all$value[
    res_df_all$type == "variation_information"])))+
  ggplot2::ggtitle(base::paste("reclustering", fraction_curr))
```
