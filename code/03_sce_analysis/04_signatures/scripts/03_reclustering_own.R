...
# get all BL6 markers
mmus_marker_list <- lapply(signature_list, function(sign){
  mmus_markers <- sign$conserved_df$gene[
    !is.na(sign$conserved_df$mmus)]
  mmus_markers <- mmus_markers[
    !mmus_markers %in% sign$subclustering_genes]
  return(mmus_markers)
})
...
clustered_list <- mclapply(sce_list, function(sce, k_graph, resolution_louvain){
