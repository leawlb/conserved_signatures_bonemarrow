...
base::table(ggdf_category_long$conservation_level[ggdf_category_long$cell_type == "Osteo"])
```

```{r}
ggplot(ggdf_category_long, aes(fill = conservation_level, x = cell_type))+
...
ggdf_category$is_marker <- vector(length = nrow(ggdf_category))
ggdf_category$is_conserved_marker <- vector(length = nrow(ggdf_category))
ggdf_category$is_conserved_signature <- vector(length = nrow(ggdf_category))

ggdf_category$is_marker[which(ggdf_category$cons_marker == FALSE &
                          ggdf_category$ == FALSE &
                          ggdf_category$ndge == FALSE)] <- TRUE

ggdf_category$is_conserved_marker[which(ggdf_category$cons_marker == TRUE &
                             ggdf_category$cons_emf == FALSE &
                             ggdf_category$ndge == FALSE)] <- TRUE

ggdf_category$is_conserved_signature[which(ggdf_category$cons_marker == TRUE &
                             ggdf_category$cons_emf == TRUE &
                             ggdf_category$ndge == TRUE)] <- TRUE

ggdf_category <- base::as.data.frame(tidyr::pivot_longer(
  ggdf_category,
  cols = c("is_marker",
           "is_conserved_marker",
           "is_conserved_signature"), 
  names_to = "mode", 
  values_to = "is_true"))
