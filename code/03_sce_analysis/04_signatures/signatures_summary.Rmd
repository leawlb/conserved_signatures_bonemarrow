...
for(i in 1:length(signature_list_hsc)){
  sign_genes <- signature_list_hsc[[i]]$conserved_signature
...
expr_df_list_hsc <- lapply(agg_hsc_list, function(agg_hsc){
  ...
  sign_genes_ct <- signature_list_hsc[[ct_curr]]$conserved_signature
...
all_sign_genes <- vector()
for(i in 1:length(signature_list_str)){
  sign_genes <- signature_list_str[[i]]$conserved_signature
  all_sign_genes <- c(all_sign_genes, sign_genes)
}
...
expr_df_list_str <- lapply(agg_str_list, function(agg_str){
  ...
  ct_curr_pos <- which(names(signature_list_str) == ct_curr)
  sign_genes_ct <- signature_list_str[[ct_curr_pos]]$conserved_signature
...
hsc_cons_df <- signature_list_hsc$HSCs$conserved_df
...
hsc_conserved_signature <- signature_list_hsc$HSCs$conserved_signature
