...
# redo dimensionality reduction for renormalized and for mnn corrected
...
sce_09mnn <- cms(sce_09mnn, group = "Batch_exp_day", k = 30,
                 assay_name = "reconstructed", res_name = "MNN")

sce_09ser <- cms(sce_09ser, group = "Batch_exp_day", k = 30, 
                 assay_name = "logcounts", res_name = paste0("Seurat ", nrow(sce_09ser)))

```

The p-val distribution should be even.
...
plot_lisi <- function(sce, title){
  ...
  plot_a <- ggplot(res, aes(x = Object_ID,))+
    geom_histogram()+
    theme_classic()+
    ggtitle(paste(spec_curr, title, nrow(sce), "Features, max nr category:", max_obj), " ")

  plot_b <- ggplot(res, aes(x = Batch_exp_day,))+
    geom_histogram()+
    theme_classic()+
    ggtitle(paste(spec_curr, title, nrow(sce),"Features, max nr category:", max_bed), " ")

  plot_c <- ggplot(res, aes(x = Batch_sequencing,))+
    geom_histogram()+
    theme_classic()+
    ggtitle(paste(spec_curr, title, nrow(sce), "Features, max nr category:",max_bsq), " ")
...
Should be closer to the maximum number of categories per variable.
Due to the observed imbalance in certain cell types per batch, perfect LISI 
scores will not be possible.
