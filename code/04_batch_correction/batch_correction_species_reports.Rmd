...
sce_00ser <- snakemake@input[["sce_09seurat"]]
...
# redo dimensionality reduction for renormalized and possiblz fastmnn corrected
...
identity_plots <- function(sce, title){
  ...
  PLOT_1 <- ggarrange(plot_1, legend_1, ncol = 1)
...
```{r fig.width=10, fig.height=5}
facet_plots_species_09mnn
```

```{r fig.width=10, fig.height=5}
facet_plots_species_09ser
```
...
```{r fig.width=10, fig.height=5}
qc_pcas_species_09mnn
```

```{r fig.width=10, fig.height=5}
qc_pcas_species_09ser
```
