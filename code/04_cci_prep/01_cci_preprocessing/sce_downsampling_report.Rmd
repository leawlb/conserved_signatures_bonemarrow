...
## 1. Cluster cell type labels and Assignment
...
qcdf_gg$Age_ID <- factor(qcdf_gg$Age_ID, levels = names(col_age))
qcdf_gg$Species_ID <- factor(qcdf_gg$Species_ID, levels = names(col_spc))
qcdf_gg$Identity <- factor(qcdf_gg$Identity, levels = names(col_cts))
qcdf_gg$Status <- factor(qcdf_gg$Status, levels = c("before downsampling",
                                                    "after downsampling"))
```
...
ggplot(qcdf_gg_bf, aes(x = Species_ID, y = sum,
  ...
  facet_grid(cols = vars(qcdf_gg_bf$Identity),
             rows = vars(qcdf_gg_bf$Age_ID))+
  theme(strip.text.x = element_text(angle = 90))+
  scale_color_manual("Species", values = col_spc)+
  ylab("Library size (ligand/receptor genes)")+
...
ggplot(qcdf_gg_bf, aes(x = Species_ID, y = detected,
  ...
  theme(strip.text.x = element_text(angle = 90))+
  scale_color_manual("Species", values = col_spc)+
  ylab("Nr of detected ligand/receptor genes")+
  ggtitle("Before downsampling")
...
qcdf_gg_yng <- qcdf_gg[qcdf_gg$Age_ID == "yng",]
ggplot(qcdf_gg_yng, aes(x = Status, y = sum,
  ...
  theme(strip.text.x = element_text(angle = 90))+
  ylab("Library size (ligand/receptor genes)")+
  ggtitle("Downsampling comparison")
```
...
qcdf_gg_yng <- qcdf_gg[qcdf_gg$Age_ID == "yng",]
ggplot(qcdf_gg_yng, aes(x = Status, y = detected,
                    color = Status, alpha = 0.5))+
  geom_jitter(size = 0.1, alpha = 50)+
  geom_violin(color = "black", alpha = 0, draw_quantiles = 0.5)+
  theme_classic()+
...
qcdf_gg_old <- qcdf_gg[qcdf_gg$Age_ID == "old",]
ggplot(qcdf_gg_old, aes(x = Status, y = detected,
                    color = Status, alpha = 0.25))+
  geom_jitter(size = 0.3, alpha = 50)+
  geom_violin(color = "black", alpha = 0, draw_quantiles = 0.5)+
  theme_classic()+
...
for(i in 1:length(sce_list)){
  ...
  print(table(duplicated(rowData(sce_temp2)$ENSMUS_ID)))
  
  sce_temp2 <- sce_temp[!duplicated(rowData(sce_temp)$ENSMUS_ID),]
  print(table(duplicated(rowData(sce_temp2)$Symbol)))
}
```
