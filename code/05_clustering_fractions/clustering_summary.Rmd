...
colData(sce_list[["mcar"]])$prediction_hierarchical <- data_test_mcar$prediction_hierarchical
colData(sce_list[["mcas"]])$prediction_hierarchical <- data_test_mcas$prediction_hierarchical
colData(sce_list[["mspr"]])$prediction_hierarchical <- data_test_mspr$prediction_hierarchical
...
cluster_plots <- function(sce){
  ...
  col_clust_h <- col_num[names(col_num) %in% sce$prediction_hierarchical]

  species_curr <- sce$Species_ID[1]
  if(species_curr != "mmus"){
    
    if(is.null(sce$prediction_hierarchical)){
      print(isnull)
    }
  
    plot_1 <- umap_base(sce, 
                        color_by = "prediction_hierarchical")+ 
      ggtitle(paste0(species_curr))+
      scale_color_manual("prediction_hierarchical", 
                         values =col_clust_h)
    plot_1l <- umap_legend(sce, color_by = "prediction_hierarchical")+
      scale_color_manual("prediction_hierarchical", 
                         values = col_clust_h)
...
sce_list[["mcar"]]$prediction_seurat <- pred_ser_mcar
sce_list[["mcas"]]$prediction_seurat <- pred_ser_mcas
sce_list[["mspr"]]$prediction_seurat <- pred_ser_mspr
...
cluster_plots <- function(sce){
  ...
  col_clust_s <- col_num[names(col_num) %in% sce$prediction_seurat]

  species_curr <- sce$Species_ID[1]
  
  if(species_curr != "mmus"){
    plot_1 <- umap_base(sce, 
                        color_by = "prediction_seurat")+ 
      ggtitle(paste0(species_curr))+
      scale_color_manual("prediction_seurat", 
                         values =col_clust_s)
    plot_1l <- umap_legend(sce, color_by = "prediction_seurat")+
      scale_color_manual("prediction_seurat", 
                         values = col_clust_s)
    legend_1 <- get_legend(plot_1l)
...
sce_list[["mcar"]]$prediction_louvain <- pred_lou_mcar
sce_list[["mcas"]]$prediction_louvain <- pred_lou_mcas
sce_list[["mspr"]]$prediction_louvain <- pred_lou_mspr
...
cluster_plots <- function(sce){
  ...
  col_clust_l <- col_num[names(col_num) %in% sce$prediction_louvain]
  ...
  if(species_curr != "mmus"){
    ...
    plot_1 <- umap_base(sce, 
                        color_by = "prediction_louvain")+ 
      ggtitle(paste0(species_curr))+
      scale_color_manual("prediction_louvain", 
                         values =col_clust_l)
    plot_1l <- umap_legend(sce, color_by = "prediction_louvain")+
      scale_color_manual("prediction_louvain", 
                         values = col_clust_l)
    legend_1 <- get_legend(plot_1l)
