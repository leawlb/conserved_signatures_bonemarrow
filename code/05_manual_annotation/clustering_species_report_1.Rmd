---
title: "clustering_species_reports1"
author: "Lea Wölbert"
date: '2022-12-19'
...
```{r}
print(table(sce_12$cluster_ref_baccin))
```

```{r}
print(table(sce_12$cluster_ref_dahlin))
```

```{r}
print(table(sce_12$cluster_ref_dolgalev))
```
...
cluster_plots_12 <- identity_plots(sce_12, title = " ")
...
ggdf <- data.frame(colData(sce_10))

plot1a <- ggplot(ggdf, aes(x = cluster_hierarchical, y=Batch_exp_day,
                          fill=Batch_exp_day, color = Batch_exp_day))+
  geom_col()+
  theme_classic()+
  scale_fill_manual("Experimental day", values = col_batch_exp_day)+
  scale_color_manual(values = col_batch_exp_day)+
...
plot2a <- ggplot(ggdf, aes(x = Batch_exp_day, y=cluster_hierarchical,
  ...
  geom_col()+
  theme_classic()+
  scale_fill_manual("Cluster (seurat)", values = col_clust_h)+
  scale_color_manual(values = col_clust_h)+
  theme(legend.position = "none", axis.text.y = element_blank(),
        axis.text.x = element_text(angle = 90))+
  ggtitle(species_curr)+
...
plot3a <- ggplot(ggdf, aes(x = cluster_hierarchical, y=Age_ID,
  ...
  geom_col()+
  theme_classic()+
  scale_fill_manual("Age", values = col_ann_age)+
  scale_color_manual(values = col_ann_age)+
  theme(legend.position = "none", axis.text.y = element_blank(),
        axis.text.x = element_text(angle = 90))+
  ggtitle(species_curr)
...
plot4 <- ggplot(ggdf, aes(x = Age_ID, y=cluster_hierarchical,
                          fill=cluster_hierarchical, 
                          color = cluster_hierarchical))+
  geom_col()+
  theme_classic()+
...
plot5a <- ggplot(ggdf, aes(x = cluster_hierarchical, y=Identity_ref_all,
  ...
  geom_col()+
  theme_classic()+
  scale_fill_manual("Identity 1", values = col_cts_all)+
...
plot6a <- ggplot(ggdf, aes(x = cluster_hierarchical, y=Identity_ref_fraction,
  ...
  geom_col()+
  theme_classic()+
  scale_fill_manual("Identity s", values = col_cts_fraction)+
...
plot6b <- ggplot(ggdf, aes(x = cluster_hierarchical, fill=Identity_ref_fraction, 
  ...
  geom_bar(position = "fill") + 
  theme_classic()+
  scale_fill_manual("Identity 2", values = col_cts_fraction)+
  scale_color_manual(values = col_cts_fraction)+
  theme(legend.position = "none", axis.text.x = element_text(angle = 90))+
  ggtitle(species_curr)
...
PLOT5a <- ggarrange(plot5a, legend_all)
PLOT5b <- ggarrange(plot5b, legend_all)
...
# Silhouette 
sil_approx <- approxSilhouette(reducedDim(sce_10, "PCA"), 
                               clusters = sce_10$cluster_hierarchical)

sil_data <- as.data.frame(sil_approx)
sil_data$closest <- factor(ifelse(sil_data$width > 0, 
                                  sce_10$cluster_hierarchical, 
                                  sil_data$other))
sil_data$cluster <- sce_10$cluster_hierarchical

ggplot(sil_data, aes(x=cluster, y=width, colour=closest))+
  ggbeeswarm::geom_quasirandom(size = 0.1)+
  theme_classic()+
  ggtitle(paste0(species_curr))+
  facet_grid(cols = vars(sce_10$Fraction_ID))+
  scale_color_manual("cluster (hierarchical)", values = col_clust_h)

# Purity
pure_sce <- neighborPurity(reducedDim(sce_10, "PCA"), sce_10$cluster_hierarchical)
pure_data <- as.data.frame(pure_sce)
pure_data$maximum <- factor(pure_data$maximum)
pure_data$cluster <- sce_10$cluster_hierarchical
...
ggplot(pure_data, aes(x=cluster, y=purity, color=cluster)) +
  ...
  ggtitle(paste0(species_curr))+
  theme_classic()+
  scale_fill_manual("cluster (hierarchical)", values = col_clust_h)
...
ggdf <- data.frame(colData(sce_11))
...
plot4 <- ggplot(ggdf, aes(x = Age_ID, y=cluster_seurat,
                          fill=cluster_seurat, color = cluster_seurat))+
  geom_col()+
  theme_classic()+
  scale_fill_manual("Cluster (seurat)", values = col_clust_s)+
...
# Silhouette
sil.approx <- approxSilhouette(reducedDim(sce_11, "PCA"), 
                               clusters = sce_11$cluster_seurat)
sil.data <- as.data.frame(sil.approx)
sil.data$closest <- factor(ifelse(sil.data$width > 0, 
                                  sce_11$cluster_seurat, 
                                  sil.data$other))
sil.data$cluster <- sce_11$cluster_seurat

ggplot(sil.data, aes(x=cluster, y=width, colour=closest))+
  ggbeeswarm::geom_quasirandom(size = 0.1)+
  theme_classic()+
  ggtitle(paste0(species_curr, sce_11$seurat_mode[1]))+
  facet_grid(cols = vars(sce_11$Fraction_ID))+
  scale_color_manual("cluster (hierarchical)", values = col_clust_h)

# Purity
pure_sce <- neighborPurity(reducedDim(sce_11, "PCA"), sce_11$cluster_seurat)
pure_data <- as.data.frame(pure_sce)
pure_data$maximum <- factor(pure_data$maximum)
pure_data$cluster <- sce_11$cluster_seurat
...
pheatmap(t(mat1), main = species_curr)
...
pheatmap(mat2, main = species_curr)
