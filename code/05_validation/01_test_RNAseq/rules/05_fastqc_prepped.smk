...
def cutadapt_output_fastq(wc):
    print(wc.read)
    if wc.read == "R1":
        return rules.cutadapt.output.fastq1.format(
            sample=wc.sample, 
            read=wc.read,
            sample_folder_name=wc.read
        )
    else:
        return rules.cutadapt.output.fastq2.format(
            sample=wc.sample, 
            read=wc.read,
            sample_folder_name=wc.read
        )
...
rule fastqc_prepped:
    input:
        trimmed_fastq 
    output:
        html = OUTPUT_BASE + "/reads/{sample_folder_name}/05_fastqc_prepped/{sample}_{read}.html",
        zip = OUTPUT_BASE + "/reads/{sample_folder_name}/05_fastqc_prepped/{sample}_{read}_fastqc.zip" # the suffix _fastqc.zip is necessary for multiqc to find the file. If not using multiqc, you are free to choose an arbitrary filename
    params:
        extra = "--quiet",
        queue = "short"  
    log:
