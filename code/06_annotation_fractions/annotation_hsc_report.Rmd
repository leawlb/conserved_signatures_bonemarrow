...
sce_hsc <- readRDS(file = "/omics/odcf/analysis/OE0538_projects/DO-0008/data/sce_objects/11_annotation/")
...
head(metadata(sce_hsc)$score_list)
head(metadata(sce_hsc)$usage_list)
head(metadata(sce_hsc)$cluster_list)

```

```{r}

plot <- umap_program(sce_hsc[,match(colnames(sce_hsc), 
                                   rownames(metadata(sce_hsc)$usage_list[["mmus"]]))],
                     species = "mmus", 
                     program = colnames(metadata(sce_hsc)$usage_list[["mmus"]])[1])
plot
