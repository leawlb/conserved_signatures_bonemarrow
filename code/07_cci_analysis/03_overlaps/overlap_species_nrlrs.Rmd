...
```{r load_objects}
#cci_mlist_mcar_yng <- readRDS("/omics/odcf/analysis/OE0538_projects/DO-0008/data/main_analysis/cci_objects/01_cci_preparation/11_idis/ilrs_mcar_yng")
#cci_mlist_mmus_yng <- readRDS("/omics/odcf/analysis/OE0538_projects/DO-0008/data/main_analysis/cci_objects/01_cci_preparation/11_idis/ilrs_mmus_yng")
#cci_mlist_mcas_yng <- readRDS("/omics/odcf/analysis/OE0538_projects/DO-0008/data/main_analysis/cci_objects/01_cci_preparation/11_idis/ilrs_mcas_yng")
#cci_mlist_mspr_yng <- readRDS("/omics/odcf/analysis/OE0538_projects/DO-0008/data/main_analysis/cci_objects/01_cci_preparation/11_idis/ilrs_mspr_yng")
...
overlap_lists <- lapply(cci_met_cts_list_cts, function(cci_met_ct){
  ...
  mmusg <- unique(cci_met_mmus$sigmol)
  mcasg <- unique(cci_met_mcas$sigmol)
  msprg <- unique(cci_met_mspr$sigmol)
  mcarg <- unique(cci_met_mcar$sigmol)

  shared <- intersect(intersect(intersect(mmusg, mcasg), msprg), mcarg)

  mmus_only <- mmusg[!mmusg %in% c(mcasg, msprg, mcarg)]
  mcas_only <- mcasg[!mcasg %in% c(mmusg, msprg, mcarg)]
  mspr_only <- msprg[!msprg %in% c(mmusg, mcasg, mcarg)]
  mcar_only <- mcarg[!mcarg %in% c(mmusg, mcasg, msprg)]
  
  all <- unique(c(mmusg, mcasg, msprg, mcarg))
  partially_shared <- all[!all %in% c(mmus_only, mcas_only, mspr_only, mcar_only, shared)]

  # count the length of each cell type entry  
  return_df <- data.frame(
    "mode" = c("shared", "mmus_only", "mcas_only", "mspr_only", "mcar_only", "partially_shared"),
    "nr" = c(length(shared), length(mmus_only), length(mcas_only),
             length(mspr_only), length(mcar_only), length(partially_shared))
