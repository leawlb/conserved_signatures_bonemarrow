...
resolution_sign <- data.frame(
  species = c("human", "marmoset", "macaque", "mouse"),
  resolution = c(0.6, 0.5, 0.5, 0.6)) # chosen in report
...
for(sp in c("human", "marmoset", "macaque", "mouse")){
  ...
  species <- FindClusters(species, 
                          resolution = resolution_sign[which(resolution_sign$species == sp), "resolution"])
  ...
  #                          dims = 1:50)
  # species <- FindClusters(species, 
  #                         resolution = resolution_marker[which(resolution_marker$species == sp), "resolution"])
  # 
  # species@commands <- list()
  # 
  # saveRDS(species,
...
species <- FindNeighbors(species,
                         ...
                         dims = 1:50)
species <- FindClusters(species, 
                        resolution = resolution_sign[which(resolution_sign$species == "mouse"), "resolution"])
