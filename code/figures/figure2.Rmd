...
Scaled expression of HSC (top), Mk prog. (middle) or Erythroid prog.
...
```{r d_prep_1}
ery_pseudo_seurat <- Seurat::CreateSeuratObject(counts = ery_pseudo@assays@data@listData[["logcounts"]],
...
plot_Mk <- plot_data[,c(colnames(plot_data)[1:6],
                           geneset_list_hsc[["Mk progs."]][["conserved_signature"]])] %>%
  gather(gene, expression,
         -pseudotime, -celltypes, -Mouse_ID, 
         -Species_ID, -Age_weeks, -Age_ID)
