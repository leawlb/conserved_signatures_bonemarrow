...
output_pdf_sign_heatmap_plot <- base::paste0(
  base_path_output_temp, "/manuscript1/figures/figure2_sign_heatmap_plot.pdf")
output_pdf_sign_heatmap_legend <- base::paste0(
  base_path_output_temp, "/manuscript1/figures/figure2_sign_heatmap_legend.pdf")

output_pdf_heatmap_spec_plot <- base::paste0(
  base_path_output_temp, "/manuscript1/figures/figure2_heatmap_spec_plot.pdf")
...
```{r sign_heatmap_assemble, fig.width = 6, fig.height = 9}
...
pdf(output_pdf_sign_heatmap_plot, width = 6, height = 8)
sign_heatmap_plot
dev.off()
```

```{r output_sign_heatmap_legend}
pdf(output_pdf_sign_heatmap_legend, width = 8, height = 4)
sign_heatmap_legend
dev.off()
#knitr::knit_exit()
```
...
## Heatmap species-specific marker rgenes
...
aging_theme_plot <- ggplot2::ggplot(
  plot_hsc,
  aes(y = celltype,
      fill = group))+
  ggplot2::geom_col(
...
```{r aging_legend, fig.width = 2, fig.height = 2}
...
pdf(output_pdf_aging_theme_plot, width = 4.5, height = 3)
aging_theme_plot
dev.off()
```

```{r output_pdf_aging_theme_legend}
pdf(output_pdf_aging_theme_legend, width = 2, height = 2)
aging_legend
dev.off()
```
...
```{r pt_umaps_legend1, fig.width = 3, fid.height = 2}
...
```{r pt_umaps_legend2, fig.width = 3, fid.height = 2}
...
pdf(output_pdf_pt_ranking_plot_long, width = 4, height = 10)
pt_ranking_plot_long
dev.off()
```
