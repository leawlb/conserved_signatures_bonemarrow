...
output_pdf_a_plot <- base::paste0(
  base_path_output_temp, "/manuscript1/figures/figure4_a.pdf")
output_pdf_a_legend <- base::paste0(
  base_path_output_temp, "/manuscript1/figures/figure4_a_legend.pdf")
output_pdf_bcd_plot <- base::paste0(
  base_path_output_temp, "/manuscript1/figures/figure4_bcd.pdf")
output_pdf_e_plot <- base::paste0(
  base_path_output_temp, "/manuscript1/figures/figure4_e.pdf")
output_pdf_e_plot_poster <- base::paste0(
  base_path_output_temp, "/manuscript1/figures/figure4_e_poster.pdf")
output_pdf_e_legend <- base::paste0(
  base_path_output_temp, "/manuscript1/figures/figure4_e_legend.pdf")
output_pdf_plots_single_poster <- base::paste0(
  base_path_output_temp, "/manuscript1/figures/figure4_heatmap_poster.pdf")
```
...
mmms_df$Var2 <- factor(
  mmms_df$Var2,
  levels = base::rev(c(1, 10, 9, 2, 4, 8, 5, 3, 6, 7)))
```

```{r sign_base_plot}
mmms_base_plot <- ggplot2::ggplot(
  mmms_df, 
  aes(x = Var1, 
      y = Var2,
      fill = Freq))+
...
```{r sign_theme_plot}
mmms_theme_plot <- mmms_base_plot+
  theme_all+
  ggplot2::theme(
    legend.position = "none",
...
mmms_df_o2$Var2 <- factor(
  mmms_df_o2$Var2,
  levels = base::rev(c(0, 3, 11, 5, 7, 10, 9, 8, 12, 6, 4, 2, 1)))
```
...
mmms_df_o3$Var2 <- factor(
  mmms_df_o3$Var2,
  levels = base::rev(c(2, 1, 7, 6, 0, 9, 8, 5, 4, 3)))
```
...
```{r assemble_poster, fig.width = 8, fig.height = 8}

mmms_plots <- ggpubr::ggarrange(
  ncol = 3,
  mmms_theme_plot+theme(axis.text.x = element_blank()),
  mmms_theme_plot_o2+theme(axis.text.x = element_blank()),
  mmms_theme_plot_o3+theme(axis.text.x = element_blank()))

mmms_theme_plot_together <- ggpubr::annotate_figure(
  mmms_plots,
  top = ggpubr::text_grob(
    "BL6 markers", 
...
sign_theme_plot_together <- ggpubr::annotate_figure(
  sign_plots,
  top = ggpubr::text_grob(
    "conserved identity signature", 
    color = col_cons_long["conserved identity signature"], 
...
plots_single_poster <- ggpubr::ggarrange(
  ncol = 1,
  align = "hv",
  heights = c(1, 2.4),
  mmms_theme_plot_together,
  sign_theme_plot_together)

```

```{r e_plot_export}
pdf(output_pdf_plots_single_poster, width = 8, height = 8)
plots_single_poster
dev.off()
```
...
# TODO: disable for not using for poster
corr_pval_df <- corr_pval_df[corr_pval_df$conservation_level != "conserved_markers",]
corr_pval_df <- corr_pval_df[corr_pval_df$type != "mean_prop_cells_cluster",]
corr_pval_df <- corr_pval_df[corr_pval_df$comparison != "mmms-vs-signrand",]
