...
output_pdf_a_plot <- base::paste0(
  base_path_output_temp, "/manuscript1/figures_new/figure5_a.pdf")
output_pdf_a_legend <- base::paste0(
  base_path_output_temp, "/manuscript1/figures_new/figure5_a_legend.pdf")
output_pdf_bcd_plot <- base::paste0(
  base_path_output_temp, "/manuscript1/figures_new/figure5_bcd.pdf")
output_pdf_e_plot <- base::paste0(
  base_path_output_temp, "/manuscript1/figures_new/figure5_e.pdf")
output_pdf_e_legend <- base::paste0(
  base_path_output_temp, "/manuscript1/figures_new/figure5_e_legend.pdf")
```
...
# all ECs
seu_sign_o2_ec <- seu_sign_o2[,seu_sign_o2$seurat_clusters %in% c("6", "1", "2", "3")]
...
DimPlot(seu_sign_o3, dims = c(1,2), group.by = "cell_type_from_publication")
...
# FBN1    related to TGFβ, MSC diff, https://pubmed.ncbi.nlm.nih.gov/26610678/
...
seu_sign_o3$annotated_reclustered[seu_sign_o3$seurat_clusters == nr] <- "Fibro CAR"
```
...
# CXCL14, EDIL3, ABI3BP, SOD3, COL6A3, ABCA8
...
seu_sign_o3$annotated_reclustered[seu_sign_o3$seurat_clusters == nr] <- "Osteo-Chondro CAR"
```
...
# FSTL1 supprssed osteo during infl https://pubmed.ncbi.nlm.nih.gov/35026647/
...
seu_sign_o3$annotated_reclustered[seu_sign_o3$seurat_clusters == nr] <- "Osteo CAR"
```
...
seu_sign_o3$annotated_reclustered[seu_sign_o3$seurat_clusters == nr] <- "Adipo CAR"
```
...
# TSPAN13, CAPG, MGST1, CD36, GPX1, PPP1R14B, CAVIN2, TXN
...
# PECAM1, FLI1, ICAM2, CRIP1, FCER1G, SH2D3C, GNGT2, LYN, IFI30, ARPC3, PLEKHO1
...
0, 11, 5, 4, 7, 10, 9, 8, 12, 13, 6, 1, 2, 3
...
seu_sign_o2$annotated_reclustered[seu_sign_o2$seurat_clusters == nr] <- "Adipo/CAR1 - batch pbs"
```
...
# scara5, Pdgfrb, Snrk, Ebf3, Ltbp3, Mgp, Hp, Foxc1, 

# scara5 adipogenesis
# Pdgfrb fibro
# ltbp3 early MSC diff, downregulated during osteo, requiored for adipo
# foxc1, highly adipo
# snrk, also adipi
# Notch3, adipo

seu_sign_o2$annotated_reclustered[seu_sign_o2$seurat_clusters == nr] <- "Adipo/CAR2 - batch pbs"
...
seu_sign_o2$annotated_reclustered[seu_sign_o2$seurat_clusters == nr] <- "Adipo/CAR - batch non-pbs"
```
...
# Alpl, Limch1, Lrp4, Dlx5, Olfml3, Sp7, 
...
# Osteo without CAR but with chondro
...
# aspn, edil3, fmod, lum, mmp2 all related to chondro
...
# Sema3c related to Itgbl1, cartilage according to wikipedia
# Prrx2 osteogenic
...
# Ackr3, Jam2, Mustn1, Cryab, Sgms2
# Ackr3 osteogenic differentiation
# mustn1 pan osteo muscule, muscle
...
seu_sign_o2$annotated_reclustered[seu_sign_o2$seurat_clusters == nr] <- "EC1 - batch non-pbs"
...
#         Dll4–Notch signalling couples sprouting angiogenesis and artery formation (https://www.nature.com/articles/ncb3555)
...
seu_sign_o2$annotated_reclustered[seu_sign_o2$seurat_clusters == nr] <- "EC2 - batch pbs"
...
seu_sign_o2$annotated_reclustered[seu_sign_o2$seurat_clusters == nr] <- "EC3 - batch non-pbs"
...
sign_df_o2$Var2 <- factor(
  sign_df_o2$Var2,
  levels = base::rev(c(
    "Adipo/CAR1 - batch pbs",
    "Adipo/CAR2 - batch pbs",
    "Adipo/CAR - batch non-pbs",
    "Osteo/CAR",
    "Osteo-Chondro",
    "Chondro1",
    "Chondro2",
...
sign_df_o2$Var1[sign_df_o2$Var1 == "O2"] <- "O2 osteo-chondro trans."
...
sign_df_o3$Var1[sign_df_o3$Var1 == "balanced prog."] <- "balanced p."
sign_df_o3$Var1[sign_df_o3$Var1 == "highly adipocytic gene-expressing progenitors"] <- "adipo-expressing p."
sign_df_o3$Var1[sign_df_o3$Var1 == "osteochondrogenic progenitors 1"] <- "osteo-chondro p. 1"
sign_df_o3$Var1[sign_df_o3$Var1 == "osteochondrogenic progenitors 2"] <- "osteo-chondro p. 2"
...
```{r assemble_bcd, fig.width = 15, fig.height =5.5}
...
```{r bcd_plot_export}
pdf(output_pdf_bcd_plot, width = 15, height = 5.5)
sign_plots
dev.off()
```
