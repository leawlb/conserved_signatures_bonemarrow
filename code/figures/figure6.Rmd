...
output_pdf_b_plot <- base::paste0(
  base_path_temp, "manuscript1/figures_new/figure6_b.pdf")
output_pdf_b_legend <- base::paste0(
  base_path_temp, "manuscript1/figures_new/figure6_b_legend.pdf")
output_pdf_c_plot <- base::paste0(
  base_path_temp, "manuscript1/figures_new/figure6_c.pdf")
output_pdf_d_plot <- base::paste0(
  base_path_temp, "manuscript1/figures_new/figure6_d.pdf")
output_pdf_e_plot <- base::paste0(
  base_path_temp, "manuscript1/figures_new/figure6_e.pdf")
```
...
factorize_mouse <- list(hs = c(2,11,10,4,6,7,3,5,8,9,0,12,1),
...
for(genes in c("hs", "sig")){
  data <- readRDS(paste0(base_path_temp, 
                         brain_path, 
...
plotted_gg_mouse[["hs"]] <- plotted_gg_mouse[["hs"]] + 
    ...
    ggplot2::theme(
      ...
      plot.title = element_text(
        ...
        face = plot_title_face,
        hjust = 0.5,
        color = col_cons_long["total human markers"])) +
...
```{r c_construct, fig.width = 5, fig.height = 6}
...
```{r c_plot_export}
pdf(output_pdf_c_plot, width = 5, height = 6)
c_plot
dev.off()
```
...
Number of mouse cortex cells that are able to be accurately re-annotated using clustering based on human marker genes or primate identity signature genes.

#### Prep

```{r d_prep1}
mouse_hs <- readRDS(paste0(base_path_temp, brain_path, 
                           "04_recluster/mouse_reclust_hs.rds"))
mouse_sig <- readRDS(paste0(base_path_temp, brain_path, 
                            "04_recluster/mouse_reclust_sig.rds"))
```
...
d_plot <- ggplot(all_data,
  ...
  theme(strip.text.y = element_text(angle = 0),
    ...
    axis.text.y = element_text(color = c(col_cons_long["conserved identity signature"], 
...
```{r d_plot_export}
pdf(output_pdf_d_plot, width = 4, height = 4)
d_plot
dev.off()
```
...
```{r e_plot_export}
pdf(output_pdf_e_plot, width = 6, height = 4)
