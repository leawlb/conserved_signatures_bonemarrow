...
ctspc_base_plot_left <- ggplot2::ggplot(
  ...
  ggplot2::facet_grid(rows = vars(hsc_umap$species))
```
...
ctspc_base_plot_right <- ggplot2::ggplot(
  ...
  ggplot2::facet_grid(rows = vars(str_umap$species))
```
...
# each section of genes per group has a different length, so determine
# by the "starting" and "ending" gene manually
...
There are at least 97 cells per species and cell types.
