...
```{r other1, fig.width = 16, fig.height = 3, eval = FALSE}
...
```{r other2, fig.width = 12, fig.height = 3, eval = FALSE}
# run the function, get max
max_o2 <- 5
...
```{r pval_plot_export}
pdf(output_pdf_a_plot, width = 12, height = 2.5)
plot_hsc
dev.off()
```
...
```{r heatmaps_combine, fig.height = 12, fig.width = 10}
...
pdf(output_pdf_c_heatmaps, width = 10.5, height = 10)
heatmaps_combine
dev.off()
```
